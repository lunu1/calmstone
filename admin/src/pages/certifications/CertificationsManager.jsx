...
function CertForm({ initial = {}, onSubmit }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (file) {
        ...
        await onSubmit({ title, href, order: Number(order)||0, isActive, image }, false);
      }
  ...
  return (
    <form onSubmit={submit} className="space-y-4" aria-busy={saving}>
      <input className="w-full rounded border px-3 py-2" placeholder="Title (e.g., ISO 9001:2015)"
      ...
      <input className="w-full rounded border px-3 py-2" placeholder="Link to certificate (optional)"
             value={href} onChange={e=>setHref(e.target.value)} disabled={saving} />
      ...
      <button className="inline-flex items-center gap-2 rounded bg-zinc-900 px-4 py-2 text-white disabled:opacity-60" disabled={saving}>
        {saving && <svg viewBox="0 0 24 24" className="h-4 w-4 animate-spin" fill="none">
...
export default function CertificationsManager() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setErr(e?.response?.data?.message || 'Failed to load certifications');
  ...
  const create = async (payload, isMultipart) => {
    try {
      if (isMultipart) {
        await api.post('/api/certifications', payload, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
  ...
  const update = async (id, payload, isMultipart) => {
    try {
      if (isMultipart) {
        await api.put(`/api/certifications/${id}`, payload, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded bg-zinc-900 px-4 py-2 text-white" onClick={() => setCreating(true)}>+ New</button>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((c) => (
          <div key={c._id} className="flex items-center gap-4 rounded-xl border bg-white p-4 shadow-sm">
            <img src={c.image} alt={c.title} className="h-12 w-auto shrink-0 rounded object-contain" />
            <div className="min-w-0 flex-1">
              <div className="truncate font-semibold">{c.title}</div>
              <div className="text-xs text-zinc-600">Order: {c.order} • Active: {String(c.isActive)}</div>
              {c.href && <div className="truncate text-xs text-blue-600">{c.href}</div>}
            </div>
            <div className="flex flex-col gap-2">
              <button className="rounded bg-zinc-900 px-3 py-2 text-white" onClick={() => setEditing(c)}>Edit</button>
              <button className="rounded bg-red-600 px-3 py-2 text-white" onClick={() => remove(c._id)}>Delete</button>
            </div>
          </div>
        ))}
      </div>

      <SimpleModal title="Create Certification" open={creating} onClose={() => setCreating(false)}>
        <CertForm onSubmit={create} />
      </SimpleModal>

      <SimpleModal title="Edit Certification" open={!!editing} onClose={() => setEditing(null)}>
        {editing && <CertForm initial={editing} onSubmit={(p, isMultipart) => update(editing._id, p, isMultipart)} />}
      </SimpleModal>
    </div>
  );
}
